...
def provision(config, client=None, *, dry_run=False):
    ...
    if output.get('platform') != 'shopee' or sheets.get('platform') != 'shopee':
        raise ValueError('仅支持 Shopee 目标初始化')
    ...
    definitions = [field_definition(key, output['fields'][key]) for key, _ in COLUMNS]
    ...
    meta = metadata()
    header_range = f'{sheet_id}!A1:AB1'
    def read_header():
    ...
    report_path = config['app']['output_dir'] / f'shopee_schema_{stamp}.json'
    report = {'status': 'preview', 'rename_primary': rename, 'create_fields': definitions[len(fields):],
              'add_columns': max(0, 28-meta['grid_properties']['column_count']),
    ...
    if not dry_run:
        ...
        if metadata()['grid_properties']['column_count'] < 28:
